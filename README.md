...
1. Client repo provides:
    ...
    - `setup.py` with `setuptools.setup()`,
      *this is the entire file:*
        ...
        setup()
        ```
    - a `pyproject.toml` file with a `[project].version` attribute (`__version__` strings in `__init__.py` files
      are disallowed). Other sections and attributes will be auto-inserted. Any conflicting
