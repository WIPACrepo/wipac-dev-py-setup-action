...
Auto-generates `setup.cfg` sections needed for publishing a package to PyPI, `README.md` badges, and `requirements.txt` (by way of [pip-compile](https://github.com/jazzband/pip-tools)). Commits are auto-pushed by the "github-actions" user (github-actions@github.com). This GitHub Action prepares a repository to be GitHub-released and PyPI-published by the [Python Semantic Release GitHub Action](https://python-semantic-release.readthedocs.io/en/latest/).
