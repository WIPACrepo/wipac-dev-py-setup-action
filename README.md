...
- the root directory's `dependencies.log` is overwritten/updated (by way of `pip freeze` + [`pipdeptree`](https://pypi.org/project/pipdeptree/)) along with dedicated `dependencies-EXTRA.txt` files for each package "extra", and
...
3. You get:
    ...
    - `dependencies.log` generated (overwritten) by `pip freeze` + [`pipdeptree`](https://pypi.org/project/pipdeptree/) using `[options].install_requires` in `setup.cfg`:
        ```
        #
