...
_MULTI_BLANK_NEWLINE_RE = re.compile(r"\n{3,}")  # 3+ -> 2
_MULTI_SPACE_INLINE_COMMENT_RE = re.compile(r" +#")  # collapse spaces before '#'
_SPACES_RE = re.compile(r"^[ \t]+", re.MULTILINE)  # leading spaces/tabs per line


def resolve_multi_blank_lines(s: str) -> str:
    """Collapse runs of blank lines to a single blank line."""
    return _MULTI_BLANK_NEWLINE_RE.sub("\n\n", s)


def resolve_multi_space_inline_comments(s: str) -> str:
    """Replace multiple spaces before '#' with a single space."""
    return _MULTI_SPACE_INLINE_COMMENT_RE.sub(" #", s)


def to_two_space_indents(s: str) -> str:
    """Normalize leading indentation to multiples of 2 spaces (floor)."""

    def _to_two_spaces(m: re.Match) -> str:
        # treat tabs as 4 spaces; adjust if you prefer a different tab width
        spaces = len(m.group(0).replace("\t", "    "))
        return "  " * (spaces // 2)

    return _SPACES_RE.sub(_to_two_spaces, s)


def normalize_toml_whitespace(s: str) -> str:
    """Run all normalizers and ensure exactly one trailing newline."""
    s = resolve_multi_blank_lines(s)
    s = resolve_multi_space_inline_comments(s)
    s = to_two_space_indents(s)
...
# Precompiled pattern:
# - ^__version__ =   → match start of line
# - re.MULTILINE     → match at start of any line
...
class FromFiles:
    ...
    def check_no_version_dunders(self) -> None:
        ...
        def commenter(match):
            ...
            return (
                f"{version_line}\n"
                f"# TODO: ^^^ remove __version__ (keep comment below)\n"
                f"{INIT_DUNDER_VERSION_NOT_NEEDED_COMMENT}\n"
            )
...
class PyProjectTomlBuilder:
    ...
    def __init__(
        ...
        self._inline_dont_change_this_comment(toml_dict["build-system"])
    ...
    def _inline_dont_change_this_comment(section: Any) -> None:
        section.trivia.comment = INLINE_DONT_CHANGE_COMMENT
...
def write_toml(
    ...
    # -- tomlkit occasionally removes the blank line before sections (not sure why)
    out = re.sub(r"^\[", r"\n[", out, flags=re.MULTILINE)  # prepend blank line
